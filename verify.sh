...
validate_scripts() {
  bash -n "$DOTFILES/bootstrap.sh" "$DOTFILES/verify.sh" "$DOTFILES/lib/restore.sh"
  bash "$DOTFILES/tests/restore.sh"
...
verify_pi() {
  ...
  [[ "$current_pi" == "$launcher" || "$(readlink -f "$current_pi")" == *"pi-coding-agent"* ]] \
